...
`RANK` is a multi-source relationship, and IYP holds hundreds of separate `Ranking` nodes — Cisco Umbrella, per-country Chrome UX Report lists, per-country eyeball estimates, CAIDA ASRank, and more. An unpinned `-[:RANK]-(:Ranking)` therefore means "appears in *any* ranking IYP carries", which is a much weaker claim than "popular" and runs straight into the warning in [Datasets](Datasets.md#writing-efficient-queries) about aggregating across sources that measure different things. It is not a harmless looseness: run the same comparison unpinned and both counts jump by roughly an order of magnitude, because a hostname now qualifies if *any* of IYP's rankings reaches it. The two networks stay in the same order, but the number has quietly stopped answering "how much popular content lives here" and started answering "how many of IYP's loaded ranking lists happen to reach into this address space" — a property of IYP's dataset inventory, not of the network.
